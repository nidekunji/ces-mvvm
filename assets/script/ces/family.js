...
var Family = module.exports = Class.extend({
    ...
    init: function (componentNames) {
        ...
        if (window.debugToyOn) {
            this.addEntityIfMatch = function (entity) {
                if (!this._entities.has(entity) && this._matchEntity(entity)) {
                    ...
                    if (consume > 0) {
                        if (this._componentNames.length === 1)
                            cc.log('on', this._componentNames[0], consume);
                        else
                            cc.log('on', this._componentNames, consume);
                    }
                }
            }
        }
